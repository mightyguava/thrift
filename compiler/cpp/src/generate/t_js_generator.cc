...
public:
  t_js_generator(t_program* program,
    ...
    iter = parsed_options.find("require_prefix");
    if (iter != parsed_options.end()) {
      require_prefix_ = (iter->second);
    }
  ...
  if (gen_node_) {
    ...
    for (size_t i = 0; i < includes.size(); ++i) {
      result += "var " + includes[i]->get_name() + "_ttypes = require('" +
        get_include_prefix(*(includes[i])) + includes[i]->get_name() + "_types')\n";
    }
    if (includes.size() > 0) {
      result += "\n";
    }
  ...
  if (gen_node_) {
    if (tservice->get_extends() != NULL) {
      f_service_ << "var " << tservice->get_extends()->get_name() << " = require('"
                 << get_include_prefix(*(tservice->get_extends()->get_program()))
...
string t_js_generator::get_include_prefix(const t_program& program) const {
  string include_prefix = program.get_include_prefix();
  if (require_prefix_.size() == 0) {
    return "./" + include_prefix;
  } else {
    return require_prefix_ + include_prefix;
  }
}
...
THRIFT_REGISTER_GENERATOR(js,
                          ...
                          "    require_prefix=     Prefix for require paths.\n")
